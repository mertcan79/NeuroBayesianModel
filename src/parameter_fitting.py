...
def fit_parameters(nodes, data):
    for node_name, node in nodes.items():
        ...
        parent_data = (
            data[[parent.name for parent in node.parents]] if node.parents else None
        )
...
def fit_continuous_parameters(node, node_data, parent_data):
    if parent_data is None:
        ...
        posterior_mean = (prior_precision * prior_mean + n * sample_mean) / (
            prior_precision + n
        )
        posterior_precision = prior_precision + n
        posterior_shape = prior_shape + n / 2
        posterior_rate = prior_rate + 0.5 * (
            n * sample_var
            + prior_precision
            * n
        ...
        posterior_mean = np.linalg.solve(
            prior_precision + n * np.linalg.inv(S),
        ...
        posterior_scale = (
            ...
            * np.outer(
                ...
                np.dot(np.linalg.inv(S), X_mean - posterior_mean),
            )
        )
