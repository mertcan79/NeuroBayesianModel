...
from archive.other.bayesian_node import BayesianNode, CategoricalNode
    ...
    def compute_sensitivity(
        ...
        try:
            ...
            for node, node_samples in samples.items():
                if node != target_node:
                    if isinstance(self.nodes[node], CategoricalNode) or isinstance(
                        self.nodes[target_node], CategoricalNode
                    ):
                        sensitivities[node] = self._compute_categorical_correlation(
                            target_samples, node_samples
                        )
                    else:
                        sensitivities[node] = np.corrcoef(target_samples, node_samples)[
                            0, 1
                        ]
