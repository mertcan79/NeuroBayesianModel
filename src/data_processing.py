...
def simple_outlier_treatment(
    data: pd.DataFrame, columns: List[str], threshold: float = 1.5
) -> pd.DataFrame:
    for col in columns:
        if col in data.columns:
            ...
            if len(col_data) == 0:
                print(
                    f"Warning: Column {col} contains only non-finite values. Skipping."
                )
                continue
            ...
            if mask.sum() != n_finite_outliers:
                print(
                    f"Warning: {mask.sum() - n_finite_outliers} non-finite outliers in column {col} were not replaced."
                )
...
def detect_and_handle_outliers(
    data: pd.DataFrame, columns: List[str], threshold: float = 3.0
) -> pd.DataFrame:
    for column in columns:
        if column in data.columns:
            ...
            data.loc[outliers, column] = clipped_values.astype(data[column].dtype)
    return data
...
def add_interaction_terms(data: pd.DataFrame, features: List[str]) -> pd.DataFrame:
    for i in range(len(features)):
        for j in range(i + 1, len(features)):
            data[f"{features[i]}_{features[j]}_interaction"] = (
...
def preprocess_data(
    data: pd.DataFrame, categorical_columns: List[str], index: str = None
) -> pd.DataFrame:
    data = data.copy()
...
def prepare_data(
    ...
    behavioral, hcp = select_features(
        behavioral, hcp, behavioral_features, hcp_features
    )
