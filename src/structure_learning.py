...
import random
from archive.other.bayesian_node import BayesianNode
...
def neurological_structure_learning(
    ...
    if prior_edges is not None:
        ...
        assert all(
            isinstance(edge, tuple) and len(edge) == 2 for edge in prior_edges
        ), "All elements in prior_edges should be tuples of length 2"
    ...
    def score_function(data, parents, node):
        ...
        if parents_data.shape[0] != node_data.shape[0]:
            raise ValueError(
                "Inconsistent number of samples between parents and node data."
            )
        ...
        if np.issubdtype(parents_data_agg.dtype, np.number) and np.issubdtype(
            node_data.dtype, np.number
        ):
            parents_data_agg = np.digitize(
                parents_data_agg,
                bins=np.linspace(min(parents_data_agg), max(parents_data_agg), num=10),
            )
            node_data = np.digitize(
                node_data, bins=np.linspace(min(node_data), max(node_data), num=10)
            )
    ...
    current_score = sum(
        score_function(data, list(G.predecessors(node)), node) for node in G.nodes()
    )
    ...
    for _ in range(iterations):
        ...
        if nx.is_directed_acyclic_graph(G_new):
            new_score = sum(
                score_function(data, list(G_new.predecessors(node)), node)
            ...
            if new_score > current_score or random.random() < np.exp(
    ...
    for node_name in data.columns:
        ...
        while len(best_parents) < max_parents:
            ...
            for potential_parent in data.columns:
                if (
                    potential_parent != node_name
                    and potential_parent not in best_parents
                ):
                    score = score_node(
                        data, node_name, best_parents + [potential_parent]
                    )
                    if score > new_score:
